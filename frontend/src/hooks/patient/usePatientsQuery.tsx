...
import type { UserPatient } from "@src/types/patient";
 ...
 */
const usePatientsQuery = (userRoleId: number) => {
  ...
  const [editPatient, setEditPatient] = useState<UserPatient | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof newPatient) => createPatient(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: typeof newPatient }) => 
  ...
  const handleEdit = useCallback((patient: UserPatient) => {
    setNewPatient({
      email: patient.email,
      password: "",
      role_id: 2,
